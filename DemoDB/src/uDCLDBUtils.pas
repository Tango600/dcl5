...
Procedure FillDatasetUpdateSQL(var Query: TDCLDialogQuery;
  {$IFDEF BDE_or_IB}{$IFNDEF SQLdbIB}var UpdateSQL: UpdateObj;{$ENDIF}
    ...
    If UserLevelLocal>ulReadOnly Then
      ...
      Begin
        ...
        Begin
          ...
          Begin
            UpdateSQL.{$IFNDEF SQLdbIB}ModifySQL{$ELSE}UpdateSQL{$ENDIF}.Text:=FindParam('modifysql=', UpdatesFields);
          End
          Else
            UpdateSQL.{$IFNDEF SQLdbIB}ModifySQL{$ELSE}UpdateSQL{$ENDIF}.Text:='update '+UpdateTable+' set '+UpdateFieldsSet+' where '+
              KeyFieldsSet;
        End;
