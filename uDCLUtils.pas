...
Begin
  ...
  Begin
    CurrPos:=0;
    While (BaseToInterface(UpperCase(Where[CurrPos]))<>BaseToInterface(UpperCase(FromString)))And
      (CurrPos+1<>Where.Count) Do
      Inc(CurrPos);
    FromPos:=CurrPos;
    While (BaseToInterface(UpperCase(Where[CurrPos]))<>BaseToInterface(UpperCase(ToString)))And
      (CurrPos+1<>Where.Count) Do
      Inc(CurrPos);
    For li:=FromPos+1 To CurrPos-1 Do
      Result.Append(BaseToInterface(Where[li]));
  End;
End;
