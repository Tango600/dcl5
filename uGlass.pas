...
procedure GlassForm(frm: TComponent);
...
type
  ...
  DwmExtendFrameIntoClientAreaFunc=function(hWnd: hWnd; const pMarInset: PMargins)
    : HRESULT; stdcall;
  SetLayeredWindowAttributesFunc=function(hWnd: hWnd; cKey: TColor; bAlpha: Byte; dwFlags: DWord)
    : BOOL; stdcall;
...
begin
  ...
  if ((GetVersionEx(osVinfo)=True)and(osVinfo.dwPlatformId=VER_PLATFORM_WIN32_NT)and
    (osVinfo.dwMajorVersion>=5) and ((osVinfo.dwMajorVersion<>6) and not (osVinfo.dwMinorVersion in [2, 3]))) then
  begin
    ...
    begin
      ...
      @fSetLayeredWindowAttributesFunc:=GetProcAddress(GetModulehandle(user32),
...
begin
  ...
  begin
    ...
    if ((bCmpEnable=True)and((MarginLeft<>0)or(MarginTop<>0)or(MarginRight<>0)or(MarginBottom<>0)or
      (SheetOfGlass=True))) then
    begin
      ...
      if SheetOfGlass=True then
        FForm.Canvas.Rectangle(0, 0, FForm.ClientWidth, FForm.ClientHeight)
      else
      begin
        if MarginLeft>0 then
          FForm.Canvas.Rectangle(0, 0, MarginLeft, FForm.ClientHeight);
        if MarginTop>0 then
          FForm.Canvas.Rectangle(0, 0, FForm.ClientWidth, MarginTop);
        if MarginRight>0 then
          FForm.Canvas.Rectangle(FForm.ClientWidth-MarginRight, 0, FForm.ClientWidth,
            FForm.ClientHeight);
        if MarginBottom>0 then
          FForm.Canvas.Rectangle(0, FForm.ClientHeight-MarginBottom, FForm.ClientWidth,
            FForm.ClientHeight);
      end;
...
begin
  ...
  if FActive=True and Assigned(FForm) then
  begin
    case Msg.Msg of
    WM_SIZE:
    UpdateForm;
    WM_NCHITTEST:
    begin
      if ((FActive=True)and(hDWMDLL<>0)and(Assigned(fDwmIsCompositionEnabled)=True)and
        (DragGlass<>dgNone)and(Msg.Result=HTCLIENT)and(IsIconic(FForm.Handle)=False)and
        (IsOnGlass=True)) then
      begin
        fDwmIsCompositionEnabled(@bCmpEnable);
        ...
        if ((bCmpEnable=True)and(wp.showCmd<>SW_HIDE)and(wp.showCmd<>SW_MAXIMIZE)and
          (wp.showCmd<>SW_MINIMIZE)) then
        begin
          ReleaseCapture;
    ...
    WM_DWMCOMPOSITIONCHANGED, WM_DWMNCRENDERINGCHANGED, WM_THEMECHANGED:
    begin
...
begin
  ...
  if ((DragGlass=dgForm)or(SheetOfGlass=True)) then
    Result:=True
  else
  begin
    GetCursorPos(cp);
    ...
    if (((MarginLeft>0)and(cp.X>=0)and(cp.X<=MarginLeft)and(cp.Y>=0)and(cp.Y<=(cr.bottom-cr.top)))or
      ...
      ((MarginRight>0)and(cp.X>=(cr.right-cr.left-MarginRight))and(cp.X<=(cr.right-cr.left))and
      (cp.Y>=0)and(cp.Y<=(cr.bottom-cr.top)))or((MarginBottom>0)and(cp.X>=0)and
      (cp.X<=(cr.right-cr.left))and(cp.Y>=(cr.bottom-cr.top-MarginBottom))and
      (cp.Y<=(cr.bottom-cr.top)))) then
      Result:=True;
  end;
end;
...
procedure GlassForm(frm: TComponent);
begin
  AeroGlass1:=TAeroGlass.Create(frm);
  AeroGlass1.DragGlass:=dgGlass;
  AeroGlass1.SheetOfGlass:=True;
end;
