...
{$ENDIF}
  ...
  uDCLMessageForm, uDCLSQLMonitor, uDCLQuery, uLogging,
...
{$ENDIF}
  ...
  public
    ...
    property GridPanel: TDCLMainPanel read FGridPanel;
  ...
  private
    ...
    Function GetFormPosString: String;
...
{$ENDIF}
  ...
  public
    ...
    procedure SetData(DataName: string; FindType: TFindType; Stream: TMemoryStream;
      Compress: Boolean);
    ...
    procedure StoreFromFile(FileName, DataName: string; FindType: TFindType; Compress: Boolean);
    procedure SaveToFile(FileName, DataName: string; FindType: TFindType);

    procedure CompressData(DataName: string; FindType: TFindType);
    procedure DeCompressData(DataName: string; FindType: TFindType);
  ...
  public
    ...
    procedure SetData(DataName: string; Stream: TMemoryStream; Compress: Boolean);
...
begin
  ...
  Begin
    ...
    Begin
      ...
      Begin
        RepParams.Append(BaseToInterface(Trim(RepParamsSet.FieldByName(GPT.DCLNameField)
          .AsString)));
        RepParams.Append(BaseToInterface(Trim(RepParamsSet.FieldByName(GPT.DCLTextField)
          .AsString)));
        RepParamsSet.Next;
      End;
    End;
...
Begin
  ...
  ChangePassForm.Width:=(BeginStepLeft*3)+EditWidth;
  ChangePassForm.Height:=(BeginStepTop*2)+EditTopStep*5-FilterLabelTop*2;
  ...
  CancelButton.Name:=AnsiToUTF8('CancelButton');
...
{$ENDIF}
  ...
  Begin
    ...
    FGrids[GridIndex].ButtonPanel.Top:=FForm.Height-1;
            ...
            Begin
              ...
              Begin
                ...
                Begin
                  ...
                  Delete(tmpSQL, Pos('=', tmpSQL), length(tmpSQL));
                End;
  ...
  Begin
    ...
    Begin
      DCLQueryL.SQL.Text:='update '+INITable+' set '+IniParamValField+'='+GPT.StringTypeChar+
        GetFormPosString+GPT.StringTypeChar+' where '+GetIniToRole(GPT.UserID)+IniDialogNameField+
    ...
    Begin
      DCLQueryL.SQL.Text:='insert into '+INITable+'('+IniUserFieldName+', '+IniDialogNameField+', '+
        ...
        GPT.StringTypeChar+', '+GPT.StringTypeChar+GetFormPosString+GPT.StringTypeChar+')';
    End;
    Try
      DCLQueryL.ExecSQL;
    Except
  ...
  Begin
    ...
    Begin
      For i:=0 To DialogsParams.Count-1 Do
        ...
        Begin
          DialogsParams[i]:=GetFormPosString;
          Yes:=true;
          break;
        End;
      If Yes=False Then
        DialogsParams.Append(GetFormPosString);
    End
    Else
      DialogsParams.Append(GetFormPosString);
            ...
            If Assigned(FDCLForm) then
              If Assigned(FDCLForm.Tables[-1].ToolPanel) Then
                ...
                Begin
                  ...
                  (FDCLForm.Tables[-1].ToolPanel.FindComponent(TmpStr) As TEdit).Text:=
                    BaseToInterface(tmpStr1);
                End;
          End;
...
{$ENDIF}
          ...
          Begin
            ...
            FDCLForm.AddStatus(BaseToInterface(TmpStr), v1);
          End;
...
{$IFDEF ADO}+'. ADO.db v.'+FDBLogOn.Version{$ENDIF})+
      AnsiToUTF8('. '+GetDCLMessageString(msStatus)+' : '+ReliseStatues[ReleaseStatus]+'.');
  ...
  Begin
    ...
    Width:=62;
    Height:=13;
    Caption:=SourceToInterface(GetDCLMessageString(msUser)+'/ '+GetDCLMessageString(msRole)+' : ')+BaseToInterface(GPT.DCLLongUserName+' ('+IntToStr(Ord(FAccessLevel))
...
{$ENDIF}
  ...
  Begin
    ...
    Width:=FormWidth-30;
    Height:=105;
    Text:=SourceToInterface(GetDCLMessageString(msConfiguration)+' : ')+BaseToInterface(GetConfigInfo)+
      SourceToInterface(' /'+GetDCLMessageString(msVersion)+' : ')+BaseToInterface(GetConfigVersion);
    Font.Color:=clWindowText;
    Font.Height:=-12;
    // Font.Style:=[fsBold, fsItalic];
    ParentFont:=False;
...
begin
  ...
  // LoadFormPos(CurrentForm, 'MainForm');
end;
...
Begin
  ...
  ItemMenu.Caption:='О...';
  // ItemMenu.OnClick:=About.ShowAboutForm;
...
Begin
  ...
  ItemMenu.Caption:=BaseToInterface(Caption);
...
Begin
  ...
  If (Key=13)and(Shift=[ssCtrl]) Then
    ...
    Begin
      ...
      Begin
        Combo.Text:=BaseToInterface(Trim(tmpQuery.FieldByName(ContextLists[ComboNum].Field)
          .AsString));
        If (FQuery.State=dsBrowse) Then
          FQuery.Edit;
        FQuery.FieldByName(ContextLists[ComboNum].DataField).AsInteger:=
      ...
      Begin
        ...
        Begin
          If Trim(tmpQuery.FieldByName(ContextLists[ComboNum].Field).AsString)<>'' Then
            Combo.Items.Append
              (BaseToInterface(Trim(tmpQuery.FieldByName(ContextLists[ComboNum].Field).AsString)));
          tmpQuery.Next;
        End;
      End;
      tmpQuery.Close;
...
Var
  ...
  Begin
    ...
    If Pos('(', Exemple)=1 Then
      If Pos(')', Exemple)=length(Exemple) Then
        OperationType:=1;

    If Pos('[', Exemple)=1 Then
      If Pos(']', Exemple)<>0 Then
        OperationType:=2;
...
begin
  ...
  FQueryGlob.Name:='Query_'+IntToStr(UpTime);
  FDCLLogOn.SetDBName(FQueryGlob);
  ...
  Begin
    if Assigned(DCLMainLogOn.Forms[v1-1]) then
      If DCLMainLogOn.ActiveDCLForms[v1-1] then
        DCLMainLogOn.Forms[v1-1].Free;
  End;

  // SaveFormPos(1, 'MainForm');
end;
...
Begin
  ...
  If DefaultSystemEncoding=EncodingUTF8 Then
    PrintBox.Append(UTF8BOM+UTF8ToAnsi(Caption))
  Else If DefaultSystemEncoding='utf16' Then
    PrintBox.Append(UTF16LEBOM+UTF8ToAnsi(Caption))
  Else
    PrintBox.Append(UTF8ToAnsi(Caption));
  ...
  Begin
    ...
    Begin
      S1:='';
      S2:=Query.Fields[FieldsCounter].FieldName;
      If length(S2)>Query.Fields[FieldsCounter].DataSize Then
        SetLength(S2, Query.Fields[FieldsCounter].DataSize+1);
      S:=S+S2;
      For k:=length(Query.Fields[FieldsCounter].FieldName) To Query.Fields[FieldsCounter]
        .DataSize Do
        S1:=S1+' ';
      S:=S+S1+'|';
    End;
    ...
    Begin
      ...
      Begin
        S1:='';
        S2:=Query.Fields[FieldsCounter].AsString;
        If length(S2)>Query.Fields[FieldsCounter].DataSize Then
          SetLength(S2, Query.Fields[FieldsCounter].DataSize+1);
        S:=S+S2;
        For k:=length(Query.Fields[FieldsCounter].AsString) To Query.Fields[FieldsCounter]
          .DataSize Do
          S1:=S1+' ';
        S:=S+S1+'|';
      End;
    ...
    Begin
      ...
      Begin
        ...
        If length(S2)>(Grid.Columns[FieldsCounter].Width Div 7) Then
          SetLength(S2, (Grid.Columns[FieldsCounter].Width Div 7)+1);
        S:=S+S2;
        For k:=length(Grid.Columns[FieldsCounter].Field.AsString)
          To (Grid.Columns[FieldsCounter].Width Div 7) Do
          S1:=S1+' ';
        S:=S+S1+'|';
      End;
      PrintBox.Append(UTF8ToAnsi(S));
      Query.Next;
    End;
...
{$ENDIF}
  ...
  If GrabFormYes Then
  Begin
    GrabValueForm.Height:=ElementsTop+BeginStepTop+ButtonPanelHeight;
...
procedure TBaseBinStore.CompressData(DataName: string; FindType: TFindType);
Var
  MS: TMemoryStream;
begin
  MS:=GetData(DataName, FindType);
  MS.Position:=0;
  SetData(DataName, FindType, MS, true);
  MS.Free;
end;
...
procedure TBaseBinStore.DeCompressData(DataName: string; FindType: TFindType);
var
  MS: TMemoryStream;
begin
  MS:=GetData(DataName, FindType);
  SetData(DataName, FindType, MS, False);
end;
...
procedure TBaseBinStore.SetData(DataName: string; FindType: TFindType; Stream: TMemoryStream;
  Compress: Boolean);
var
  BS: TMemoryStream;
  tmpSQL: String;
...
begin
  ...
  Begin
    ...
    Case FindType of
    ftByName:
    tmpSQL:='select * from '+FTableName+' where '+GPT.UpperString+GPT.TemplatesNameField+
      GPT.UpperStringEnd+'='+GPT.UpperString+GPT.StringTypeChar+DataName+GPT.StringTypeChar+
      GPT.UpperStringEnd;
    ftByIndex:
...
procedure TBaseBinStore.StoreFromFile(FileName, DataName: string; FindType: TFindType;
  Compress: Boolean);
var
  MS: TMemoryStream;
begin
  ...
  Begin
    ...
    SetData(DataName, FindType, MS, Compress);
  End;
end;
...
procedure TBinStore.CompressData(DataName: string);
begin
  inherited CompressData(DataName, FFindType);
end;
...
procedure TBinStore.DeCompressData(DataName: string);
begin
  inherited DeCompressData(DataName, FFindType);
end;
...
procedure TBinStore.SetData(DataName: string; Stream: TMemoryStream; Compress: Boolean);
begin
  inherited SetData(DataName, FFindType, Stream, Compress);
end;

procedure TBinStore.StoreFromFile(FileName, DataName: string; Compress: Boolean);
begin
  inherited StoreFromFile(FileName, DataName, FFindType, Compress);
end;
